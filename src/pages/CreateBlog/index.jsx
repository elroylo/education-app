    ...
    return (
        <Formik
            ...
            onSubmit={async (values, helpers) => {
                let coverFile = new Moralis.File(cover.name, cover)
                await coverFile.saveIPFS()
                let url = coverFile.url()
                    let headers = new Headers()
                    headers.append('Content-Type', 'application/json')
                    helpers.setSubmitting(true)
                    fetch('https://civileducation.epiccodewizard2.repl.co/blogs/add', {
                        method: 'POST',
                        body: JSON.stringify({
                            title: values.title,
                    ...
                    <div>
                        ...
                        <form onSubmit={handleSubmit} className='m-3 p-3 shadow-md border border-gray-200 rounded-lg'>
                            <div className='text-center space-y-2 p-2'>
                                ...
                                <p>
Submit a Blog! Educate the public about your favorite topics. Share you knowledge from topics such as productivity including time blocking and a pomodoro timer to current event topics
                                </p>
                            </div>
                            <div className='space-y-3 p-2'>
                                ...
                                <div>
                                    ...
                                    <input name='content' className='outline-none w-3/5 w-xl max-w-xl px-4 py-2 border border-gray-400 rounded-lg' onChange={handleChange} value={values.content} />
                                </div>
                                <div>
                                    <p className='py-2 text-left justify-left font-semibold text-xl'>
                                        Cover:
                                    </p>
                                    <input type='file' name='cover' className='outline-none w-3/5 bg-gray-200 font-bold' onChange={(e) => setCover(e.target.files[0])} />
                                </div>
                            </div>
